              ...
              <div className="order-body">
                ...
                <ul>
                  {order.items.map((item) => {
                    ...
                    // The map() function iterates over each item in the 'order.items' array.
                    // For each 'item', it returns an <li> element that displays the item's details.
                    // The 'key' prop is important for React to efficiently update the list.
                    return (
                      <li key={item.menuItem._id}>
                        {item.menuItem.name} ({item.size}) - Qty:{" "}
                        {item.quantity} - Price:{" "}
                        {/* Assuming 'item.menuItem.sizes' is an object or Map */}
